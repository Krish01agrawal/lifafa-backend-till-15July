...
# ============================================================================
# NEW: 6-MONTH HISTORICAL EMAIL FETCH WITH REAL-TIME WEBSOCKET UPDATES
# ============================================================================

@router.websocket("/ws/historical-sync")
async def websocket_historical_sync(websocket: WebSocket):
    """
    🚀 SOLUTION: WebSocket endpoint for 6-month historical email fetch with real-time progress
    ...
    try:
        ...
        # 3. Start 6-month historical sync with REAL-TIME progress updates
...
async def historical_sync_with_realtime_progress(client_id: str, user_id: str, access_token: str):
    ...
    try:
        ...
        from .gmail import email_extractor
        ...
        await manager.send_progress_update(
            client_id, 
            "complete", 
            f"✅ 6-month sync complete! {result.get('emails_processed', 0)} emails processed", 
            100,
            {
...
async def fetch_historical_emails_with_progress(service, user_id: str, client_id: str):
    ...
    try:
        ...
        for i in range(0, len(all_messages), batch_size):
            ...
            # Fetch batch details sequentially to avoid memory issues
            for msg in batch:
                try:
                    ...
                    logger.error(f"Error fetching email {msg.get('id')}: {e}")
                    continue
            ...
            await manager.send_progress_update(
                client_id, 
                "fetching_details", 
                f"Processed {i + len(batch)}/{len(all_messages)} emails ({len(detailed_emails)} successful)...", 
                int(progress)
            )
        ...
        logger.error(f"Error in fetch_historical_emails_with_progress: {e}")
        return []
...
async def process_emails_with_websocket_progress(user_id: str, emails: list, client_id: str):
    ...
    try:
        ...
        for i in range(0, len(emails), chunk_size):
            ...
            # Progress update
            progress = 50 + ((i / len(emails)) * 35)  # 50-85%
            await manager.send_progress_update(
                client_id, 
                "processing_chunk", 
                f"Processing emails {i + 1}-{min(i + chunk_size, len(emails))} of {len(emails)}...", 
                int(progress)
            )
            
            # Process chunk
            result = await process_and_store_emails(user_id, chunk, is_historical=True)
            
            if result.get("success"):
                total_processed += result.get("emails_stored", 0)
        ...
        await manager.send_progress_update(
            client_id, 
            "processing_complete", 
            f"Email processing complete: {total_processed} emails stored", 
            85
        )
        ...
        return {
            ...
            "financial_transactions": 0  # Will be calculated separately
        }
        
    except Exception as e:
        logger.error(f"Error processing emails with progress: {e}")
        return {
            "success": False,
            "error": str(e),
            "emails_processed": 0
...
async def handle_websocket_connection(websocket: WebSocket, provided_chat_id: str = None):
    ...
    try:
        ...
        try:
            ...
            if not token:
                logger.warning(f"❌ {client_id} - No JWT token provided")
